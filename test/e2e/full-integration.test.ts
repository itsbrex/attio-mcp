...
describe('End-to-End Integration Tests', () => {
  ...
  beforeAll(() => {
    ...
    if (!process.env.ATTIO_API_KEY) {
      console.warn(
        '⚠️ ATTIO_API_KEY not set - tests will use mocked responses'
      );
    }
  });
  ...
  describe('Full Flow with Cache Testing', () => {
    it('should complete full search and fetch flow with cache disabled', async () => {
      ...
      const fetchResult = await fetch(
        'people:12345678-1234-1234-1234-123456789abc'
      );
    ...
    it('should complete full flow with cache enabled and demonstrate cache hits', async () => {
      ...
      const fetch1 = await fetch(
        'companies:comp-12345678-1234-1234-1234-123456789abc'
      );
  ...
  describe('Performance Under Load', () => {
    it('should handle concurrent requests efficiently', async () => {
      ...
      const promises = Array(10)
        .fill(null)
        .map((_, i) => search(`concurrent-${i}`, 'tasks'));
    ...
    it('should manage memory efficiently with large datasets', async () => {
      ...
      console.log(
        `Memory growth after processing 50 large responses: ${memoryGrowth.toFixed(2)} MB`
      );
